...
function run_pipeline_for_various_lambda(N::Int, lambda_values::Vector{Float64}, num_simulations::Int, initialPval, initialtauval)
    ...
    df = DataFrame(Lambda_Values = λ_vals_to_plot, Tau_0_Values = τ_0_values, IsIrreducible = isirreducible_values)
    ...
    create_movie(overall_movie_folder, joinpath(overall_folder, movie1*".mp4"), frame_rate=10)
    create_movie(overall_movie_folder_2, joinpath(overall_folder,movie2*".mp4"), frame_rate=10)
