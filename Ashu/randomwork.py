...
D_i=np.array(D_i)
for i in range(n_s):
    D_i[i,n_p:n_p+n_s, :n_p] = Ai[i]              # Insert A_i 
    D_i[i,0:n_p, -1] = lmbda * (alphai[i]).reshape(1,n_p)  # Insert lambda * alpha_i 
    D_i[i,-1, -1] = 1 

S_0=np.zeros((n_t, n_t))
S_0[n_p:n_p+n_s, :n_p] = A_tilde_0            # Insert A_0tilde
S_0[0:n_p, -1] = lmbda * (alphai[0]).reshape(1,n_p)  # Insert lambda * alpha_0
S_0[-1, -1] = 1 
